...
function loadButtons(cycles) {
    let output = `<button onclick=first()>First</button> <button onclick=previous()>Previous</button>`;
    for (let i = 0; i < cycles; i++) {
        output += `<button id=${i} onclick=paginate(${i})>${i + 1}</button>`;
    }

    output += `<button onclick=next()>Next</button>`;
...
function previous() {
    ...
    current_id - 1 < 0 ? paginate(9) : paginate(current_id - 1);
}
